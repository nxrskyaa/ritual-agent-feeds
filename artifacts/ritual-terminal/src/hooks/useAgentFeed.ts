  ...
  return {
    ...
    timestamp: new Date(
      Number(msg.timestamp) > 1e12
        ? Number(msg.timestamp)          // already milliseconds
        : Number(msg.timestamp) * 1000   // seconds → milliseconds
    ).toISOString(),
    type: 'user',
    status: 'confirmed',
...
export function useAgentFeed() {
  ...
  // Read: Get the required POST_FEE from contract
  const getPostFee = useCallback(async (): Promise<bigint> => {
    try {
      const client = getPublicClient()
      const fee = await client.readContract({
        address: CONTRACT_ADDRESS as `0x${string}`,
        abi: CONTRACT_ABI,
        functionName: 'POST_FEE',
      })
  ...
  const postMessage = useCallback(
    async (content: string): Promise<{ hash: string; entry: FeedEntry }> => {
      ...
      try {
        ...
        // Fetch the required fee before posting
        ...
        if (error.message?.includes('IncorrectFee')) {
          throw new Error('Incorrect fee sent with message')
        }
        throw error
      } finally {
        setIsPosting(false)
